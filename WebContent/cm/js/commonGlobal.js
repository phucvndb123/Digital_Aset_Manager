...
// Template-safe switches for new projects.
// Keep generic helpers enabled and disable WRM-specific business integrations by default.
gcm.templateConfig = {
...
gcm.win.getActiveWindowInfo = function() {
	...
	if (popupWindow !== null) {
		...
		if (activeInfo["type"] == "T") {
			...
			if (gcm.templateConfig.enableWrmMenuDataset
				&& !com.util.isEmpty($p.main().wfm_layout)
				&& !com.util.isEmpty(gcm.data.getParameter($p.main().wfm_layout.getWindow().$p, "menuInfo"))
				&& !com.util.isEmpty(gcm.data.getParameter($p.main().wfm_layout.getWindow().$p, "menuInfo").programCd)) {
...
gcm.win.openMenu = function($p, menuNm, url, menuCode, paramObj, option) {
	...
	if (com.util.isEmpty(url)) {
		...
		if (layout == "T") {
			...
			if (isClose) {
				if (gcm.templateConfig.enableWrmMenuDataset
					&& !com.util.isEmpty($p.main().wfm_side)
					&& !com.util.isEmpty($p.main().wfm_side.getWindow().dlt_menu)) {
...
gcm.win.changePageState = function() {
	if(!com.util.isEmpty(history.state) && !com.util.isEmpty(history.state.data) && !com.util.isEmpty(history.state.data.menuInfo)){
		...
		if (gcm.templateConfig.enableHistoryMenuSync
			&& !com.util.isEmpty($p.main().wfm_side)
			&& !com.util.isEmpty($p.main().wfm_side.getWindow().trv_menu)
			&& typeof $p.main().wfm_side.getWindow().trv_menu.findNodeByValue === "function") {
